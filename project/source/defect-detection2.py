...
def train_loop(train_loader, model, criterion, optimizer):
    ...
    torch.save(model, os.path.join(current_location, "saved-deeplab2.pth")) #saving the training so can train again later
    state = {
        'state_dict': model.state_dict(),
        'optimizer': optimizer.state_dict()
    } #### what is this???????

    torch.save(state, os.path.join(current_location, "saved-deeplab-checkpoint2.pth"))
...
def load_checkpoint_and_train_and_save_model():
    ...
    checkpoint = torch.load(os.path.join(current_location, "saved-deeplab-checkpoint2.pth"))
...
def test_model():
    ...
    model = torch.load(os.path.join(current_location, "saved-deeplab2.pth")) ### maybe putting it in a place???
...
def test_model_accuracy():
    ...
    model = torch.load(os.path.join(current_location, "saved-deeplab2.pth")) #loading the locaiton of the deeplab and all the weights and biases
